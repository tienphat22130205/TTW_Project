    ...
    public class InvoiceDetailDao {
        ...
        public static List<CartProduct> getInvoiceDetails(int invoiceId) {
            ...
            String sql = """
            SELECT d.id_product, p.product_name, d.price, d.quantity, d.item_discount
        ...
        """;
            ...
            try (PreparedStatement ps = DbConnect.getPreparedStatement(sql, true)) {
                ...
                while (rs.next()) {
                    ...
                    item.setId_product(rs.getInt("id_product")); // ✅ thêm dòng này
        ...
        public static void main(String[] args) {
            ...
            if (details.isEmpty()) {
                ...
                for (CartProduct item : details) {
                    System.out.println("---------------");
                    System.out.println("🆔 Mã sản phẩm: " + item.getId_product());
                    System.out.println("🛒 Tên sản phẩm: " + item.getName());
                    System.out.println("🔢 Số lượng: " + item.getQuantity());
                    System.out.println("💵 Giá gốc: " + item.getPrice() + " đ");
                    System.out.println("🔻 Giảm giá: " + item.getDiscount() + " %");
                    double subtotal = item.getPrice() * item.getQuantity() * (1 - item.getDiscount() / 100);
